    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 md:gap-6">
        {stats.map((stat, index) => {
          ...
          return (
            <div key={index} className="p-6 transition-shadow duration-200 bg-white border border-gray-100 shadow-sm rounded-xl hover:shadow-md">
          ...
          {user?.role === 'admin' ? (
            <>
              ...
              <button className="flex items-center p-4 transition-colors duration-200 rounded-lg bg-green-50 hover:bg-green-100">
                ...
                <span className="font-medium text-green-700">Review Deposits</span>
              </button>
            </>
          ) : (
            <>
